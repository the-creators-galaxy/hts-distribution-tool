...
export async function createDistributionScenario(associatedCount: number, autoAssociatedCount: number, unassociatedCount: number) {
	...
	for (let i = 0; i < autoAssociatedCount; i++) {
		accountsInfo.push(await createAccount(0, 1, 1, 5));
	}
	for (let i = 0; i < unassociatedCount; i++) {
		accountsInfo.push(await createAccount(0, 1, 1, 0));
	}
	...
	return {
		treasuryInfo,
		tokenInfo,
		distributionPayerInfo,
		accountsInfo,
		distributionFilePath,
	};
}

export async function createAccount(initialBalance: number = 0, keyCount: number = 1, keyThreshold: number = 1, maxAutoAssociations: number = 0) {
